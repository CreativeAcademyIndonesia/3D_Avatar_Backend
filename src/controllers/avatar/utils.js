...
const lipSyncMessage = async (message) => {
  ...
  const outputPath = path.join(projectRoot, 'storage', 'avatar', 'audios', `message_${message}.json`);
  const inputPath = path.join(projectRoot, 'storage', 'avatar', 'audios', `message_${message}.wav`);
